...
func (n *DiscoveryDefault) Load(_ cfacade.IApplication) {
	...
	if nodeConfig.LastError() != nil {
		logrus.Error("`node` property not found in profile file.")
		return
	}
	...
	for _, nodeType := range nodeConfig.Keys() {
		...
		for i := 0; i < typeJson.Size(); i++ {
			...
			if nodeId == "" {
				logrus.Errorf("nodeId is empty in nodeType = %s", nodeType)
				break
			}

			if _, found := n.GetMember(nodeId); found {
				logrus.Errorf("nodeType = %s, nodeId = %s, duplicate nodeId", nodeType, nodeId)
				break
			}
