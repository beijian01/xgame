...
func (f *SourceFile) Init(_ IDataConfig) {
	...
	if err != nil {
		logrus.Panicf("Unmarshal fileConfig fail. err = %v", err)
		return
	}
	...
	if err != nil {
		logrus.Panicf("AddFilterHook extName fail. err = %v", err)
		return
	}
	...
	if err != nil {
		logrus.Panicf("[name = %s] join path fail. err = %v.", f.Name(), err)
		return
	}

	err = f.watcher.Add(f.monitorPath)
	if err != nil {
		logrus.Panicf("New watcher error. path=%s, err = %v", f.monitorPath, err)
		return
	}
...
func (f *SourceFile) ReadBytes(configName string) ([]byte, error) {
	...
	if len(data) < 1 {
		return nil, cerr.Errorf("RawMsg is empty. [configName = %s]", configName)
	}
			...
			case ev := <-f.watcher.Event:
				{
					...
					logrus.Infof("Trigger file change. [name = %s]", configName)

					data, err := f.ReadBytes(configName)
					if err != nil {
						logrus.Warn("Read data fail. [name = %s, err = %s]", configName, err)
						continue
					}
...
func (f *SourceFile) Stop() {
	...
	logrus.Infof("Remote watcher [path = %s, err = %v]", f.monitorPath, err)
