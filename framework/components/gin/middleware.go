...
import (
	csync "github.com/beijian01/xgame/framework/extend/sync"
	...
	return func(c *Context) {
		defer func() {
			if err := recover(); err != nil {
				...
				if brokenPipe {
					logrus.Warnw(c.Request.URL.Path,
						"error", err,
				...
				if stack {
					logrus.Warnw("[Recovery from panic]",
						"time", time.Now(),
						"error", err,
						"request", string(httpRequest),
...
func MaxConnect(n int) GinHandlerFunc {
	...
	return func(c *Context) {
		if latch.TryBorrow() {
			...
			logrus.Warnf("limit = %d, service unavailable. url = %s", n, c.Request.RequestURI)
